...
const BottomNav: React.FC<BottomNavProps> = ({ currentView, navigate }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 w-full max-w-md mx-auto bg-background/90 backdrop-blur-sm border-t border-accent/20 z-20">
        ...
        {navItems.map(({ view, label, icon: Icon }) => (
          <button
            ...
            className={`flex flex-col items-center justify-center w-full h-full transition-colors duration-200 ${
              currentView === view ? 'text-income' : 'text-text-main/60'
            }`}
