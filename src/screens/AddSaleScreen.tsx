...
const QuantityControl: React.FC<QuantityControlProps> = ({ quantity, onIncrease, onDecrease }) => (
    <div className="flex items-center space-x-3">
        <button onClick={onDecrease} className="bg-secondary/30 text-text-main w-8 h-8 rounded-full font-bold text-lg" style={{minWidth: '32px', minHeight: '32px'}}>-</button>
        <span className="font-mono text-xl w-10 text-center">{quantity}</span>
        <button onClick={onIncrease} className="bg-primary text-white w-8 h-8 rounded-full font-bold text-lg" style={{minWidth: '32px', minHeight: '32px'}}>+</button>
    </div>
);
...
const AddSaleScreen: React.FC<AddSaleScreenProps> = ({ navigate }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
        ...
        <footer className="fixed bottom-0 left-0 right-0 w-full max-w-md mx-auto bg-background border-t border-secondary/20 p-4">
