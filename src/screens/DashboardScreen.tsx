...
const Header: React.FC<HeaderProps> = ({ onProductsClick, onLogoutClick }) => {
  ...
  return (
    <header className="sticky top-0 bg-background/80 backdrop-blur-sm z-10 p-4 flex justify-between items-center border-b border-secondary/20">
      <div>
      ...
      <div className="flex items-center space-x-2">
        <button onClick={onProductsClick} className="p-2 rounded-full hover:bg-secondary/20 transition-colors" aria-label="Gestionar productos">
          <CupcakeIcon className="w-6 h-6 text-secondary" />
        </button>
        <button onClick={onLogoutClick} className="p-2 rounded-full hover:bg-secondary/20 transition-colors" aria-label="Cerrar sesión">
            <LogOutIcon className="w-6 h-6 text-secondary" />
        </button>
      </div>
    </header>
  );
    ...
    <div className="fixed bottom-20 right-6 z-20">
      <div className={`flex flex-col items-center space-y-3 transition-all duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
        ...
        <button onClick={onAddExpense} className="bg-white p-3 rounded-full shadow-lg flex items-center space-x-2 text-sm text-text-main hover:bg-gray-100 transition-colors">
            ...
            <CartIcon className="w-6 h-6 text-alert" />
        </button>
      </div>
      <button
        onClick={handleFabClick}
        className="bg-primary text-white w-16 h-16 rounded-full shadow-lg flex items-center justify-center mt-3 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary/50"
                    ...
                    todaysTransactions.map(t => (
                        <div key={t.id} className="bg-white/50 p-3 rounded-lg flex items-center justify-between shadow-sm">
                            <div className="flex items-center space-x-3">
                                <div className={`w-2 h-10 rounded-full ${t.type === 'sale' ? 'bg-primary' : 'bg-alert'}`}></div>
                                <div>
                            ...
                            <div className="flex items-center space-x-3">
                                <p className={`font-mono font-bold ${t.type === 'sale' ? 'text-green-600' : 'text-red-600'}`}>
...
const DashboardScreen: React.FC<DashboardScreenProps> = ({ navigate }) => {
  ...
  const balanceColor = dailyTotals.balance >= 0 ? 'bg-background' : 'bg-alert/20';
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 space-y-4 flex-grow">
        <StatCard title="Vendí Hoy" amount={dailyTotals.sales} colorClass="bg-primary/20" />
        <StatCard title="Gasté Hoy" amount={dailyTotals.expenses} colorClass="bg-alert/20" />
